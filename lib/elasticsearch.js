...
exports.init = function elasticsearch_init(startup_time, config, events, logger) {
  ...
  if (configEs.host.match('http')) {
    uri_parse = url(elasticHost);
    elasticHost           = uri_parse.hostname || 'localhost';
    elasticPort           = uri_parse.port     || 9200;
  } else {
    console.log("HTTP Scheme not defined in Host");
